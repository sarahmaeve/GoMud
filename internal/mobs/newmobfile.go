...
func CreateNewMobFile(newMobInfo Mob, copyScript string) (MobId, error) {
	...
	if err := fileloader.SaveFlatFile[*Mob](configs.GetFilePathsConfig().DataFiles.String()+`/mobs`, &newMobInfo, saveModes...); err != nil {
		return 0, err
	}
	...
	if copyScript != `` {
		...
		sampleScriptRoot := configs.GetFilePathsConfig().SampleScripts.String()
		fileloader.CopyFileContents(
			util.FilePath(sampleScriptRoot, `/`, `mobs/`, copyScript),
